...
bcftools annotate --threads 32 \
	...
	-c INFO/gnomAD_AF \
	-O z \
	-o ${sample}_somatic_filtered.gnomad.vcf.gz \
	${sample}_somatic_filtered.vcf.gz

bcftools index ${sample}_somatic_filtered.gnomad.vcf.gz

#rm ${sample}_somatic_filtered.vcf.gz*

# Filter population frequency threshold: 0.0001
bcftools filter -e 'INFO/gnomAD_AF >= 0.0001 && INFO/gnomAD_AF != "."' \
	-s "CommonVariant" -m+ \
	-O z \
	-o ${sample}_somatic_filtered.gnomad.filtered.vcf.gz ${sample}_somatic_filtered.gnomad.vcf.gz

bcftools index ${sample}_somatic_filtered.gnomad.filtered.vcf.gz

# Annotate with SnpEff
java -jar $SNPEFF_JAR ann -v hg38 ${sample}_somatic_filtered.gnomad.filtered.vcf.gz | \
...
# Remove variants with VAF > 40%
bcftools filter -s "HighVAF" -m+ -e 'FORMAT/AF > 0.4' ${sample}_mutect.vcf > ${sample}_mutect_vafFiltered.vcf
...
conda run -n pytmb pyTMB.py \
	-i ${sample}_mutect_vafFiltered.vcf \
	--sample ${sample} \
	--effGenomeSize 1700000 \
